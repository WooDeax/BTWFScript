...
local function firePrompt(prompt)
    if fireproximityprompt then
        -- Madium: работает в свёрнутом окне Windows
        fireproximityprompt(prompt)
    elseif prompt and prompt.InputHoldBegin then
        -- Delta: работает в фоне на Android
        pcall(function()
            prompt:InputHoldBegin()
            task.wait((prompt.HoldDuration or 1) + 0.02)
    ...
    while task.wait(0.1) do
        ...
        if _G.AutoSawDistance and Stones.Value > 2 then
            if currentPrompt then
                ...
                if tick() - lastSawFireTime >= 2.0 then
                    task.spawn(function()
