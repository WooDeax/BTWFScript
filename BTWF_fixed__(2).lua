...
-- VirtualInputManager недоступен на Delta/Android — безопасная загрузка
...
ScreenGui.Parent = (coreGuiOk and coreGui) or LocalPlayer:WaitForChild("PlayerGui")
...
-- DRAG: работает и на ПК (Mouse) и на телефоне (Touch)
...
local lastSawFireTime = 0


-- Активация станка: Madium (ПК) и Delta (Android)
-- Активация промпта: использует InputHoldBegin (найдено шпионом)

local function firePrompt(prompt)
    if not prompt or not prompt.Parent then return end

    -- Способ 1: Madium/Synapse (ПК, работает в свёрнутом окне)
    if fireproximityprompt then
        fireproximityprompt(prompt)
        return
    end

    -- Способ 2: Delta (Android) - InputHoldBegin
    pcall(function()
...
task.spawn(function()
    while task.wait(0.1) do
        ...
        if _G.AutoSawDistance then
            if currentPrompt then
                -- Каждый тик принудительно держим настройки промпта
                pcall(function()
                    if not savedDistances[currentPrompt] then
                        savedDistances[currentPrompt] = currentPrompt.MaxActivationDistance
                    end
                    currentPrompt.MaxActivationDistance = 9999
                    if savedLOS[currentPrompt] == nil then
                        savedLOS[currentPrompt] = currentPrompt.RequiresLineOfSight
                    end
                    currentPrompt.RequiresLineOfSight = false
                end)

                if holdingPrompt ~= currentPrompt then
                    holdingPrompt = currentPrompt
                    lastSawFireTime = 0
                end

                if tick() - lastSawFireTime >= 1.5 then
                    task.spawn(function()
                        pcall(function() firePrompt(currentPrompt) end)
                    end)
                    lastSawFireTime = tick()
                end
